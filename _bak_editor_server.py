...
ACTIONS = [
    ...
    {"action": "发送表情", "category": "聊天", "icon": "🖼️",
     ...
     "params": [{"key": "表情", "label": "表情图片路径", "type": "text",
                ...
                {"key": "发送后", "label": "发完后底部面板去向", "type": "select",
                 "options": ["自动", "键盘", "收起"], "default": "自动"}]},
    ...
    {"action": "对方后台发表情", "category": "聊天", "icon": "🖼️",
     ...
     "params": [{"key": "联系人", "label": "目标会话联系人", "type": "text", "default": ""},
                {"key": "表情", "label": "表情图片路径", "type": "text",
                 "placeholder": "如：/images/myemoji/xxx.png", "default": ""}]},
    {"action": "发送emoji", "category": "聊天", "icon": "😀",
     "desc": "我方发送 emoji 小表情（微信新版 3D 黄脸表情，弹笑脸面板→逐个点选→发送，全真机动画）。"
             "表情写名称/别名/编号，多个用逗号分隔（逐个进输入框后一次发送）。"
             "全部可用名称及含义见 /images/wxemoji3d/names.json（微笑/呲牙/大笑/爱心/捂脸/握手/墨镜/"
             "害羞/皱眉/花痴/流汗/生气/羞涩/闭嘴/惊恐/装酷/大哭/调皮/憨笑/哭泣/委屈/愤怒/难过/尴尬/"
             "抓狂/惊讶/无语/困/恐惧，或 1~29 编号，或「随机」）",
     "params": [{"key": "表情", "label": "emoji 名称/编号", "type": "text",
                 "placeholder": "如：害羞 / 呲牙 / 3 / 3,5,8 / 随机", "default": ""},
                {"key": "发送后", "label": "发完后底部面板去向", "type": "select",
                 "options": ["自动", "键盘", "收起"], "default": "自动"}]},
    {"action": "对方emoji", "category": "聊天", "icon": "😀",
     "desc": "对方发来 emoji 小表情（直接上屏左侧气泡）。表情写名称/编号/随机，"
             "可用名称见 /images/wxemoji3d/names.json",
     "params": [{"key": "表情", "label": "emoji 名称/编号", "type": "text",
                 "placeholder": "如：捂脸 / 5 / 随机", "default": ""}]},
    ...
    {"action": "切换底部面板", "category": "聊天", "icon": "🎛️",
     "desc": "直接切换聊天页底部面板（键盘/表情/更多/收起），不退回聊天底部；"
             "输入栏 230ms 对称 S 曲线直接位移，旧面板下滑、新面板同帧上滑（复刻"
             "参考视频「微信聊天框三个界面切换.mp4」）",
     "params": [{"key": "面板", "label": "目标面板", "type": "select",
                 "options": ["键盘", "表情", "更多", "收起"], "default": "键盘"},
                {"key": "停留", "label": "切换后停留秒数（可空，默认 0.3）", "type": "number",
                 "default": "0.3"}]},
    {"action": "面板切换序列", "category": "聊天", "icon": "🎞️",
     "desc": "按给定节奏「一口气」连续切换底部三面板（键盘/表情/更多/收起），"
             "精确定时复刻参考视频的整段切换；定时在浏览器端完成，节奏可逐帧对齐。"
             "序列每项 = {面板, 停留}：立刻切到「面板」，停留 N 秒再执行下一项。",
     "params": [{"key": "序列", "label": "序列（JSON 数组，元素含 面板/停留）", "type": "textarea",
                 "default": "[{\"面板\":\"收起\",\"停留\":1.83},"
                            "{\"面板\":\"键盘\",\"停留\":1.75},"
                            "{\"面板\":\"表情\",\"停留\":1.25},"
                            "{\"面板\":\"更多\",\"停留\":1.7},"
                            "{\"面板\":\"收起\",\"停留\":1.65},"
