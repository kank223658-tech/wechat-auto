...
两条路径：
  1. LLM 路径：调用 DeepSeek chat/completions（OpenAI 兼容，标准库 urllib），
     由 deepseek-v4-flash 翻译剧本并自动补全仿真动作；
...
ACTION_SCHEMA = [
    ...
    {"action": "我方打字", "desc": "手机键盘动画逐字打字 + 回车发送", "params": [{"key": "内容", "label": "消息内容"}, {"key": "插话", "label": "对方插话（边打字边对面发，可空）"}]},
    {"action": "打字不发", "desc": "打出文字但不发送，停在输入框展示", "params": [{"key": "内容", "label": "消息内容"}, {"key": "停留", "label": "停留秒数"}, {"key": "插话", "label": "对方插话（打字后对面逐条回，可空）"}]},
    {"action": "删除文字", "desc": "退格删除输入框字符（-1 清空）", "params": [{"key": "数量", "label": "删除字符数"}]},
    {"action": "对方正在输入", "desc": "聊天头部显示「对方正在输入...」", "params": [{"key": "秒数", "label": "显示秒数"}]},
    {"action": "对方发消息", "desc": "对方消息以左侧气泡整句上屏", "params": [{"key": "内容", "label": "消息内容"}, {"key": "头像", "label": "对方头像（可空）"}]},
    {"action": "对方后台发消息", "desc": "给「当前不在看的会话」投递一条对方消息，画面不变，仅把该会话在主页的预览+未读角标刷新（之后返回主页可看到）；内容以 [图片] 开头时变成图片气泡", "params": [{"key": "联系人", "label": "目标会话联系人"}, {"key": "内容", "label": "消息内容（[图片] 开头=图片消息）"}, {"key": "图片", "label": "配图路径（图片消息用，可空）"}, {"key": "头像", "label": "对方头像（可空）"}, {"key": "发送者", "label": "群聊发送者名（可空）"}, {"key": "置顶", "label": "来消息置顶（是/否，可空默认是）"}]},
    ...
    {"action": "向下滚动", "desc": "自然滚动指定像素", "params": [{"key": "像素", "label": "像素值"}]},
    ...
    {"action": "编辑朋友圈", "desc": "按数据重建朋友圈动态列表", "params": [{"key": "数据", "label": "JSON 数组"}]},
...
def cap_home_and_align_contacts(steps: list, warnings: list = None) -> list:
    ...
    # 2) 收集所有 [打开聊天] 的联系人（按出现顺序去重）
    contacts = []
    seen = set()
    for s in steps:
        if isinstance(s, dict) and s.get("action") == "打开聊天":
            n = str((s.get("params") or {}).get("联系人", "")).strip()
            if n and n not in seen:
                seen.add(n)
                contacts.append(n)
    ...
    for n in contacts:
        ...
        if queue:
            ...
            warnings.append(
                f"[打开聊天] 联系人「{n}」不在历史会话里，且主页前 9 个联系人都已被占用，"
                f"请把它加入历史会话块的前 9 位。")
    if mapping:
        for s in steps:
            if isinstance(s, dict) and s.get("action") == "打开聊天":
        ...
        for old, new in mapping.items():
            warnings.append(
                f"[打开聊天] 联系人「{old}」不在历史会话里，已对齐到主页人物「{new}」，"
                f"保证打开的人与主页显示一致。若不想被替换，请把它写进历史会话块。")
    return steps
...
def _line_to_message(line: str):
    ...
    内容以 [语音] 开头 -> kind=voice；以 [图片]/[配图] 开头 -> kind=image；否则 kind=text。
...
def split_history_block(text: str, pre_mapping: dict = None):
    ...
    for i, raw in enumerate(lines):
        ...
        if head:
            ...
            cur_name = (head.group(1) or "").strip()
            cur_messages = []
            continue
        ...
        if _BRACKET_LEAD_RE.match(line):
            # 行首是 [动作指令]：历史块到此结束，该行保留给剧本
            clean_start = i
            break
        # 普通对白行
        msg = _line_to_message(raw)
