...
text = open(r"F:\weixin-auto\_stall_user.txt", encoding="utf-8").read()
...
for i, s in enumerate(result["steps"][:12], 1):
    print(f"  {i:>2} {s.get('action')} {json.dumps(s.get('params'), ensure_ascii=False)[:120]}")
print("WARNINGS:", list(warnings), list(hist_warnings))

with open(r"F:\weixin-auto\_stall_workflow.json", "w", encoding="utf-8") as fh:
    json.dump({"name": "stall", "steps": result["steps"]}, fh, ensure_ascii=False, indent=2)
print("已写入 _stall_workflow.json")
