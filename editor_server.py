...
# 微信界面本身的 UI 素材目录 -> 归类为「图标库」；其余目录归类为「图像库」（用户可挑选的内容图片）。
# 分类仅用于前端筛选展示，不改动磁盘文件。调整此集合即可自定义分类。
...
ACTIONS = [
    ...
    {"action": "编辑朋友圈", "category": "朋友圈", "icon": "🖼️",
     "desc": "按数据重建动态列表（昵称/文案/图片/点赞/评论全可配）",
     "params": [{"key": "数据", "label": "JSON 数组或 .json 文件路径", "type": "textarea",
                 "placeholder": '[{"author":"夜华","text":"...","images":["/images/header/yehua.jpg"],"likes":["夜华"]}]',
                 "default": ""}]},
...
def _build_scene_workflow(scene):
    """把 scene 转成一段可运行的工作流 steps（我的资料 + 主页会话 + 朋友圈）。"""
    scene = scene or {}
    steps = []
    if scene.get("me"):
        me = scene["me"]
...
def _save_upload_image(data_url, name="", folder=AVATAR_DIR_NAME):
    ...
    **真实字节**并按真实格式归一化重编码（透明图保 PNG、其余转 RGB JPEG、
    ...
    defname = "avatar" if folder == AVATAR_DIR_NAME else "bg"
    fname = (f"{stem}_{stamp}{ext}" if stem else f"{defname}_{stamp}{ext}")
...
def _parse_script_to_steps(text, offline=False):
    ...
    # 离线解析专用：主页一屏最多 9 个会话，只保留前 9 个历史会话、丢弃后续；
