...
- git 推送：直连被重置，**必须走 git 已配置的代理 127.0.0.1:7899**（`-c http.proxy=` 清空反而失败）。⚠️大包推送静默挂起的真凶=**凭据助手链挂起**（helper-selector/GCM，代理吞吐实测 1.7MB/s 没问题）；解法：从 GCM 取出 token（`git-credential-manager get`，秒回）内嵌 push URL 绕过助手链，50MB 约半分钟推完。token 只进命令行，不落盘不入库。
