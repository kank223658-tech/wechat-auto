# -*- coding: utf-8 -*-
"""复现实验：setMomentsPosts 与 滚动(.wpm-scroll) 两个真实运行专属变量的作用。"""
import os, sys
...
def run(pw, chrome, name, set_posts, do_scroll):
    ...
    info = page.evaluate("""() => {
        ...
        const sc = document.querySelector('#moments .wpm-scroll') || document.getElementById('moments');
        return { rect: [r.x, r.y, r.width, r.height], st: sc.scrollTop,
                 cover: !!document.querySelector('#moments .moments__cover, #moments .cover') };
    }""")
    page.screenshot(path=os.path.join(OUT, "rep_%s.png" % name))
    br.close()
    print(name, info)
...
with sync_playwright() as pw:
    ...
    run(pw, chrome, "S1_posts_scroll", True, True)
    run(pw, chrome, "S2_scroll_only", False, True)
    run(pw, chrome, "S3_posts_noscroll", True, False)

import numpy as np
from PIL import Image
cover = np.asarray(Image.open(os.path.join(OUT, "true_cover.png")).convert("RGB").resize((196,196))).astype(np.int16)
for name in ["S1_posts_scroll", "S2_scroll_only", "S3_posts_noscroll"]:
    im = Image.open(os.path.join(OUT, "rep_%s.png" % name)).convert("RGB")
    info_rect = None
    # 贴片位置按各自 dump 的 rect——滚动后 y 不同；这里直接重新量化两种可能位置
    best = None
    for box in [(1038, 972, 1038+336, 972+336), (1038, 2232, 1038+336, 2232+336)]:
        a = np.asarray(im.crop(box).resize((196,196))).astype(np.int16)
        d = np.abs(a - cover).mean()
        if best is None or d < best[1]: best = (box, d)
    print("%-18s 最佳位置 %s 平均差 %.1f" % (name, best[0], best[1]))
